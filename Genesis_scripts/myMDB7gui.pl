...
# zmienic nazwe skryptu na inspecta_result.pl

# todo:
# moj output ma byc kopiom outputu z inspecty (*.csv)
...
my $myInspectaPath = '\\\\' . $myInspectaHost . '\\inspecta\\Access\\';
my $myInspectaUser = 'pfa';
my $myInspectaPass = 'pfa';
...
# make the %myXrayCoords unique by delete of the same rows (comparing by coordinates)
	...
	my $myLayersPath = $ENV{GENESIS_TMP} . "/" . $myLayersFile;
		$text->insert( "end", " --> myLayersPath: $myLayersPath \n" );
	...
	my $myPanelCounter =
	  ( $#myLinear + 1 ) / 10 / $myLayerCounter;    # 10 is the number of columns
	if ( $myPanelCounter == 0 ) {
		$text->insert( "end",
	...
	# every layer has 8 meassured coordinates: X11 - Y11 ...
	my $numberCoords = 8;
	for ( my $k = 0 ; $k < $myLayerCounter ; $k++ ) {
		for ( my $i = 0 ; $i < $numberCoords ; $i++ ) {
			...
			$j = $i + ( $k * $numberCoords );
			do {
				$myLinear2[$j] =~ s/,/./g;    # replace a comma by a dot
...
sub myCalcBtnCmd {
	...
	# kucha w polozeniu plika csv (temp)
	# my $myQuadPath = $ENV{GENESIS_TMP} . "/" . substr( $myMDBName, 0, -4 ) . "_Quadrotti.csv";
	# $text->insert( "end", "myLocalQuadFile: " . $myLocalQuadFile . "\n" );
...
sub chosen_mdbBE {
	...
	# convertion mdb2csv if Layers.csv does'nt exist
	...
	# and get the file list
	my @liste = readdir($myVERZ);
	$text->insert( "end", "znalazlem plikow: " . $#liste . "\n");
	closedir $myVERZ;
	...
	# tutaj zapisywac przekonwertowane pliki csv
	# "$ENV{GENESIS_TMP}/layers.csv"
	# "$ENV{GENESIS_TMP}/quadrotti.csv"
	
	my $destPath = $ENV{GENESIS_TMP} . "/" . $tempName;
	$text->insert( "end", "destPath: " . $destPath . "\n");
	...
	# $text->insert( "end", "myQuadPath: " . $myQuadPath . "\n");
...
sub getMDBList {
	...
	# choose mdb-files corresponding to the jobnumber
	my $mySubFile = "";
	for ( my $i = 0 ; $i <= $listeLn ; $i++ ) {
		$mySubMDBFile = substr( $liste[$i], 0, 8 );
